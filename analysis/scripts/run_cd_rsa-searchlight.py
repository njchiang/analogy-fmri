...
def run_subject(sub, maskname="graymatter-bin_mask",
    ...
    selector = labels[(labels.CD == 1) & (labels.Match == "1")].sort_values([
        "SubRel", "TrialTag"])
    ...
    results = pa.searchlight_rsa(
        fmri_data,
        model_rdms[(model_rdms.type == "avg")].dropna(axis=1).values[:, 2:],
        m=mask,
        write=False,
        logger=logger,
        **options["searchlight"])
    ...
    pu.data_to_img(results.scores_, bg_image).to_filename(
        os.path.join(paths["root"], "analysis", sub,
                     "multivariate", "rsa",
                     "{}_{}_CD_corr_rsaSearchlight.nii.gz".format(sub,
                                                                  maskname)))
    return results
...
def main(argv):
    ...
    logger = pu.setup_logger(os.path.join(paths['root'], 'analysis', sub,
                                          'multivariate', "rsa"),
                             fname="{}_CD-pearson.log".format(roi))
